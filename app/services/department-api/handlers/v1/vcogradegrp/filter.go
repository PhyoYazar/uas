package vcogradegrp

import (
	"net/http"

	"github.com/PhyoYazar/uas/business/core/vcograde"
	"github.com/PhyoYazar/uas/business/sys/validate"
	"github.com/google/uuid"
)

func parseFilter(r *http.Request) (vcograde.QueryFilter, error) {
	values := r.URL.Query()

	var filter vcograde.QueryFilter

	if studentID := values.Get("student_id"); studentID != "" {
		id, err := uuid.Parse(studentID)
		if err != nil {
			return vcograde.QueryFilter{}, validate.NewFieldsError("student_id", err)
		}
		filter.WithStudentID(id)
	}

	if subjectID := values.Get("subject_id"); subjectID != "" {
		id, err := uuid.Parse(subjectID)
		if err != nil {
			return vcograde.QueryFilter{}, validate.NewFieldsError("subject_id", err)
		}
		filter.WithSubjectID(id)
	}

	if year := values.Get("year"); year != "" {
		filter.WithYear(year)
	}

	if academicYear := values.Get("academic_year"); academicYear != "" {
		filter.WithAcademicYear(academicYear)
	}
	if err := filter.Validate(); err != nil {
		return vcograde.QueryFilter{}, err
	}

	return filter, nil
}

// =============================================================================
